...
- recommend
  - due to [#455](https://github.com/GoogleCloudPlatform/spark-on-k8s-operator/issues/455), the helm is recommended.
    ...
    - [installation](https://github.com/GoogleCloudPlatform/spark-on-k8s-operator/blob/master/docs/quick-start-guide.md#installation)
    - [helm installation guide](https://github.com/helm/charts/tree/master/incubator/sparkoperator)

      ```bash
      $ helm install incubator/sparkoperator --namespace spark-operator --set enableWebhook=true --set sparkJobNamespace=default --set serviceAccounts.spark.name=spark --generate-name
      ```

      or

      ```bash
      # helm install <name> <repo/release> <flag>
      # this enable the installCrd
      $ helm install spark-operator-k8s incubator/sparkoperator --namespace spark-operator --set enableWebhook=true --set sparkJobNamespace=spark-job --set installCrds=true
      ```

    - mind the service account

      ```bash
      $ kubectl get sa
      # NAME                             SECRETS   AGE
      # default                          1         13d
      # sparkoperator-1594806612-spark   1         143m   -----> this one
      ```
...
- [spark-operator dockerfile](https://github.com/GoogleCloudPlatform/spark-on-k8s-operator/blob/master/spark-docker/Dockerfile)

## API

- [API-DOC](https://github.com/GoogleCloudPlatform/spark-on-k8s-operator/blob/master/docs/api-docs.md)
...
- create a new headless service for pyspark [reference](./jupyter-svc.yaml) (note: for every notebook it is different headless service)
...
- [spark configuration v2.4.5](https://spark.apache.org/docs/2.4.5/configuration.html)
- [spark configuration k8s v2.4.5](https://spark.apache.org/docs/2.4.5/running-on-kubernetes.html)

## known issue

- [#4306](https://github.com/kubeflow/kubeflow/issues/4306)
- [#455](https://github.com/GoogleCloudPlatform/spark-on-k8s-operator/issues/455)